...
EXPERIMENT = "main"
SEEDS = [0, 1, 2]
CONFIG_PATH = "exp_configs/dr_exp_wrappers"
...
def submit_job(config_name, seed, priority):
    ...
    cmd = [
        ...
        "--config-name", f"{config_name}_wrapper",
        f"seed={seed}",
...
for step_idx, step_name in enumerate(STEPS):
    for seed_idx, seed in enumerate(SEEDS):
        ...
        priority = (len(STEPS) - step_idx) * 100 + (2 - seed_idx) * 10
...
print("\nTo launch workers (3 workers on 1 GPU):")
print(f"  uv run dr_exp --base-path {EXP_DIR} --experiment {EXPERIMENT} launcher --workers-per-gpu 3")