...
from dr_exp.mock.supabase_mock_client import SupabaseMockClient # Assuming this is the correct path
...
def test_add_job_and_get_details(mock_client, sample_job_config, sample_sweep_config_id):
    """Tests adding a new job and retrieving its details and config."""
    added_job = mock_client.add_job(sample_job_config, sample_sweep_config_id, status="queued")
...
def test_claim_job_multiple_queued(mock_client, sample_job_config, sample_sweep_config_id):
    """Tests claiming one of multiple queued jobs."""
    job1 = mock_client.add_job(sample_job_config, sample_sweep_config_id, status="queued")
    job2_config = {"learning_rate": 0.01}
    job2 = mock_client.add_job(job2_config, "sweep_cfg_xyz789", status="queued")
    ...
    assert claimed_job1["id"] != claimed_job2["id"] # Make sure different jobs were claimed
...
def test_upload_artifact_file(mock_client, tmp_path, sample_job_config, sample_sweep_config_id):
    ...
    result = mock_client.upload_artifact(job_id, str(local_artifact_file), remote_suffix)
    assert result["success"] is True
    
    expected_destination_path = os.path.join(mock_client.mock_storage_path, f"run_{job_id}", "artifacts", remote_suffix)
...
def test_upload_artifact_root_file(mock_client, tmp_path, sample_job_config, sample_sweep_config_id):
    ...
    expected_destination_path = os.path.join(mock_client.mock_storage_path, f"run_{job_id}", remote_suffix)
    assert os.path.exists(expected_destination_path)
    assert result["storage_path"] == expected_destination_path
    with open(expected_destination_path, 'r') as f:
...
def test_upload_artifact_directory(mock_client, tmp_path, sample_job_config, sample_sweep_config_id):
    ...
    result = mock_client.upload_artifact(job_id, str(local_artifact_source_dir), remote_suffix)
    assert result["success"] is True

    expected_destination_base = os.path.join(mock_client.mock_storage_path, f"run_{job_id}", "artifacts", remote_suffix)
    assert os.path.exists(expected_destination_base)
    assert os.path.exists(os.path.join(expected_destination_base, "file1.txt"))
    assert os.path.exists(os.path.join(expected_destination_base, "subdir", "file2.txt"))
    with open(os.path.join(expected_destination_base, "file1.txt"), 'r') as f:
        assert f.read() == "content1"


def test_upload_artifact_non_existent_local(mock_client, sample_job_config, sample_sweep_config_id):
    """Tests uploading a non-existent local artifact."""
    added_job = mock_client.add_job(sample_job_config, sample_sweep_config_id)
    job_id = added_job["id"]
    
    result = mock_client.upload_artifact(job_id, "non_existent_file.txt", "some_remote_path.txt")
    assert result["success"] is False
    assert "not found" in result["message"].lower()

def test_finalize_job(mock_client, sample_job_config, sample_sweep_config_id):
    """Tests finalizing a job."""
    added_job = mock_client.add_job(sample_job_config, sample_sweep_config_id, status="running")
    job_id = added_job["id"]
...
def test_multiple_operations_on_same_job(mock_client, tmp_path, sample_job_config, sample_sweep_config_id):
    ...
    mock_client.update_job(job_id, {"heartbeat": datetime.now(timezone.utc).isoformat()})
    ...
    mock_client.upload_artifact(job_id, str(local_artifact_file), "checkpoints/final_model.pt")

    # 7. Finalize job
    final_meta = {"final_val_acc": 0.9, "upload_complete_at": datetime.now(timezone.utc).isoformat(), "finalize_success": True}
    ...
    artifact_path = os.path.join(mock_client.mock_storage_path, f"run_{job_id}", "artifacts", "checkpoints/final_model.pt")
