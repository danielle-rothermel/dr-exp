...
def test_validate_priority() -> None:
    ...
    # Invalid priorities
    with pytest.raises(AssertionError, match="Priority must be between"):
        validate_priority(-1)

    with pytest.raises(AssertionError, match="Priority must be between"):
        validate_priority(1001)

    with pytest.raises(AssertionError, match="Priority must be an integer"):
...
def test_validate_job_id() -> None:
    ...
    # Invalid job IDs
    with pytest.raises(AssertionError, match="Job ID cannot be empty"):
        validate_job_id("")

    with pytest.raises(AssertionError, match="Job ID cannot be empty"):
        validate_job_id("   ")

    with pytest.raises(AssertionError, match="Job ID must be at least 8 characters"):
        validate_job_id("short")

    with pytest.raises(AssertionError, match="Job ID must be a string"):
...
def test_validate_positive_int() -> None:
    ...
    # Invalid values
    with pytest.raises(AssertionError, match="test must be positive"):
        validate_positive_int(0, "test")

    with pytest.raises(AssertionError, match="test must be positive"):
        validate_positive_int(-1, "test")

    with pytest.raises(AssertionError, match="test must be an integer"):
...
def test_validate_job_statuses() -> None:
    ...
    # Invalid statuses
    with pytest.raises(AssertionError, match="Status list cannot be empty"):
        validate_job_statuses([])

    with pytest.raises(AssertionError, match="Invalid job statuses"):
...
def test_validate_config_overrides() -> None:
    ...
    # Invalid overrides
    with pytest.raises(AssertionError, match="Invalid override format"):
        validate_config_overrides("invalid_format")

    with pytest.raises(AssertionError, match="Empty key in override"):
