...
@pytest.mark.integration  
def test_debug_health_check_healthy_system():
...
@pytest.mark.integration
def test_debug_health_check_with_issues():
...
@pytest.mark.integration
def test_debug_health_check_verbose():
...
@pytest.mark.integration
def test_configuration_mismatch_detection(tmp_path):
...
@pytest.mark.integration
def test_enhanced_worker_diagnostics_integration(tmp_path):
    ...
    with patch.dict(os.environ, env_vars):
        ...
        exit_code = main(["system", "run_worker", "test_worker", str(tmp_path / "work")])
        # Should return 1 (no job available) but show diagnostics
        assert exit_code == 1


@pytest.mark.integration 
def test_worker_diagnostics_with_jobs(tmp_path, isolated_job_db):
...
@pytest.mark.integration
def test_end_to_end_diagnostic_workflow(tmp_path):
    ...
    with patch.dict(os.environ, env_vars):
        ...
        worker_exit = main(["system", "run_worker", "test_worker", str(tmp_path / "work")])
        assert worker_exit == 1  # No jobs available


@pytest.mark.integration
def test_configuration_validation_in_health_check():
...
@pytest.mark.integration
def test_debug_commands_with_different_modes():
...
@pytest.mark.integration
def test_stale_job_detection_in_health_check(tmp_path, enhanced_mock_time):
...
@pytest.mark.integration
def test_worker_alternative_location_detection():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch.dict(os.environ, env_vars):
            exit_code = main(["system", "run_worker", "test_worker", os.path.join(alt_path, "work")])
