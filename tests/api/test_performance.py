...
def test_response_time_benchmarks(client: Any) -> None:
    ...
    for endpoint, max_time in endpoints_with_limits:
        ...
        assert response_time < max_time, (
            f"{endpoint} took {response_time:.3f}s (limit: {max_time}s)"
        )
    ...
    for i, job in enumerate(jobs):
        for j in range(3):  # 3 concurrent boosts per job
            ...
            thread = threading.Thread(
                target=boost_priority, args=(thread_id, job["id"])
            )
            threads.append(thread)
...
def test_pagination_performance_with_large_dataset(client: Any, db_client: Any) -> None:
    ...
    for test_case in pagination_tests:
        ...
        assert response_time < 3.0, (
            f"Pagination too slow for per_page={test_case['per_page']}: {response_time:.3f}s"
        )
...
def test_filtering_performance(client: Any, db_client: Any) -> None:
    ...
    for filter_url in filter_tests:
        ...
        assert response_time < 5.0, (
            f"Filtering too slow for {filter_url}: {response_time:.3f}s"
        )
...
def test_rate_limiting_behavior(client: Any) -> None:
    ...
    assert max_response_time < 2.0, (
        f"Slowest response too slow: {max_response_time:.3f}s"
    )
...
def test_database_connection_pooling(client: Any, db_client: Any) -> None:
    ...
    def database_intensive_operation(operation_id: int) -> List[Any]:
        ...
        for i in range(5):
            job = create_test_job(
                db_client, sweep_config_id=f"db_test_{operation_id}_{i}"
            )
