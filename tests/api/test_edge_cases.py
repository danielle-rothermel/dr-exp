...
def test_malformed_json_requests(client, db_client, admin_headers):
    ...
    for payload in malformed_payloads:
        ...
        assert resp.status_code in [400, 422], f"Unexpected response for payload: {payload}"
...
def test_missing_required_fields(client, admin_headers):
    ...
    for endpoint, payload in missing_field_requests:
        ...
        assert resp.status_code in [400, 404, 422], f"Should reject missing fields for {endpoint}: {payload}"
...
def test_invalid_field_types(client, db_client, admin_headers):
    ...
    for endpoint, payload in invalid_type_requests:
        resp = client.post(endpoint, json=payload, headers=admin_headers)
        assert resp.status_code in [400, 422], f"Should reject invalid types for {endpoint}: {payload}"
...
def test_out_of_range_values(client, db_client, admin_headers):
    ...
    out_of_range_requests = [
        ("/job/boost-priority", {"job_id": job_id, "boost_amount": -100}),  # Negative boost
        ("/job/boost-priority", {"job_id": job_id, "boost_amount": 10000}),  # Too large boost
        ("/job/set-priority", {"job_id": job_id, "priority": -1}),  # Below minimum
        ("/job/set-priority", {"job_id": job_id, "priority": 1001}),  # Above maximum
        ("/job/set-priority", {"job_id": job_id, "priority": 1.5}),  # Float instead of int
    ]
    
    for endpoint, payload in out_of_range_requests:
        resp = client.post(endpoint, json=payload, headers=admin_headers)
        assert resp.status_code in [400, 422], f"Should reject out-of-range values for {endpoint}: {payload}"
...
def test_nonexistent_job_operations(client, admin_headers):
    ...
    for endpoint, payload in operations:
        resp = client.post(endpoint, json=payload, headers=admin_headers)
        assert resp.status_code in [404, 400], f"Should handle nonexistent job for {endpoint}"
...
def test_extremely_long_strings(client, admin_headers):
    ...
    long_string_requests = [
        ...
        ("/job/set-priority", {"job_id": "valid-id", "priority": 500, "reason": extremely_long_string}),
    ]
    
    for endpoint, payload in long_string_requests:
        resp = client.post(endpoint, json=payload, headers=admin_headers)
        # Should either accept it or reject with 400/422, but not crash
        assert resp.status_code in [200, 400, 404, 422], f"Should handle long strings gracefully for {endpoint}"
...
def test_unicode_and_special_characters(client, db_client, admin_headers):
    ...
    for special_string in special_strings:
        ...
        assert resp.status_code in [200, 400], f"Should handle special characters: {special_string}"
...
def test_concurrent_operations_on_same_job(client, db_client, admin_headers):
    ...
    for operation_id, status_code, response in results:
        assert status_code in [200, 404, 400], f"Unexpected status for operation {operation_id}"
...
def test_pagination_edge_cases(client, db_client):
    ...
    for params in edge_case_params:
        ...
        assert resp.status_code in [200, 400, 422], f"Should handle edge case params: {params}"
...
def test_request_without_content_type(client, admin_headers):
    ...
    headers_without_content_type = {k: v for k, v in admin_headers.items() if k.lower() != "content-type"}
...
def test_empty_and_whitespace_only_strings(client, db_client, admin_headers):
    ...
    for edge_string in edge_case_strings:
        ...
        resp = client.post(
            "/job/kill",
            json={"job_id": edge_string},
            headers=admin_headers
        )
        assert resp.status_code in [400, 404, 422], f"Should handle edge string as job_id: '{repr(edge_string)}'"
        ...
        assert resp.status_code in [200, 400], f"Should handle edge string as reason: '{repr(edge_string)}'"