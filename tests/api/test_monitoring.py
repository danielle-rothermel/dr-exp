...
def test_metrics_endpoint_job_counts(client: Any, db_client: Any) -> None:
    ...
    _queued_jobs = [
        create_test_job(db_client, status=JobStatus.QUEUED) for _ in range(3)
    ]
    _running_jobs = [
        create_test_job(db_client, status=JobStatus.RUNNING) for _ in range(2)
    ]
    _completed_jobs = [
        create_test_job(db_client, status=JobStatus.COMPLETED) for _ in range(1)
    ]
    ...
    for endpoint in excluded_endpoints:
        ...
        if resp.status_code == 200:  # Skip if endpoint doesn't exist in test
            assert "X-API-Deprecation-Notice" not in resp.headers, (
                f"Endpoint {endpoint} should not have deprecation headers"
            )
            assert "X-API-Migration-Guide" not in resp.headers, (
                f"Endpoint {endpoint} should not have migration headers"
            )
