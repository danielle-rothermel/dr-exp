        ...
        if resp.status_code == 200:
            for header_name, expected_value in expected_security_headers.items():
                assert header_name in resp.headers, (
                    f"Missing security header {header_name} on {endpoint}"
                )
                assert resp.headers[header_name] == expected_value, (
                    f"Incorrect {header_name} value on {endpoint}"
                )
    ...
    for endpoint in all_endpoints:
        ...
        if resp.status_code == 200:
            assert "x-api-version" in resp.headers, (
                f"Missing API version header on {endpoint}"
            )
            assert resp.headers["x-api-version"] == expected_version, (
                f"Incorrect API version on {endpoint}"
            )
    ...
    for endpoint in endpoints:
        ...
        if resp.status_code == 200:
            assert "x-process-time" in resp.headers, (
                f"Missing timing header on {endpoint}"
            )
            ...
            assert 0 <= process_time <= 10.0, (
                f"Unreasonable process time {process_time} on {endpoint}"
            )
    ...
    for endpoint in legacy_endpoints:
        ...
        if resp.status_code == 200:
            assert "x-api-deprecation-notice" in resp.headers, (
                f"Missing deprecation notice on {endpoint}"
            )
            assert "x-api-migration-guide" in resp.headers, (
                f"Missing migration guide on {endpoint}"
            )
    ...
    for endpoint in current_endpoints:
        ...
        if resp.status_code == 200:
            assert "x-api-deprecation-notice" not in resp.headers, (
                f"Unexpected deprecation on {endpoint}"
            )
            assert "x-api-migration-guide" not in resp.headers, (
                f"Unexpected migration guide on {endpoint}"
            )
    ...
    for endpoint in endpoints:
        ...
        if resp.status_code == 200:
            assert "content-length" in resp.headers, (
                f"Missing content-length on {endpoint}"
            )
            ...
            assert content_length == actual_length, (
                f"Content-length mismatch on {endpoint}"
            )
...
def test_headers_on_error_responses(client: Any) -> None:
    ...
    for header in expected_headers:
        assert header in resp.headers, (
            f"Missing security header {header} on error response"
        )
