...
def test_worker_infinite_polling_no_jobs() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert not result.worker_finished, "Worker should not finish with infinite polling"
        assert result.no_job_count >= 1, f"Expected at least 1 no_job poll, got {result.no_job_count}"
        assert result.exception is None, f"Worker should not raise exception: {result.exception}"


def test_worker_max_jobs_termination() -> None:
    """Test that worker terminates after processing max_jobs."""
    with tempfile.TemporaryDirectory() as tmpdir:
        job_db = JobDB(base_path=tmpdir, experiment_name="max_jobs_test", validate=False)
        ...
        assert result.stats["total"] == 2, f"Expected 2 jobs processed, got {result.stats['total']}"
        assert result.stats["completed"] == 2, f"Expected 2 completed, got {result.stats['completed']}"
        assert result.exception is None, f"Worker should not raise exception: {result.exception}"
...
def test_worker_polling_with_sync_enabled() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        job_db = JobDB(base_path=tmpdir, experiment_name="sync_polling_test", validate=False)
...
def test_worker_thread_lifecycle_during_polling() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert final_threads <= initial_threads + 1, f"Thread leak detected: {final_threads} vs {initial_threads}"
        
        # Verify threads were created and stopped
        assert worker.heartbeat_thread is not None, "Heartbeat thread should have been created"
        assert not worker.heartbeat_thread.is_alive(), "Heartbeat thread should be stopped"


def test_worker_shutdown_signal() -> None:
    """Test that worker responds to shutdown signals in background threads."""
    with tempfile.TemporaryDirectory() as tmpdir:
        job_db = JobDB(base_path=tmpdir, experiment_name="shutdown_test", validate=False)
