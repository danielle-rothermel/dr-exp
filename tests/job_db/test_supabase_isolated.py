...
def isolated_supabase_client(test_namespace):
    ...
    def tracked_add_config(cluster_id, config_json, config_hash, interface_version=None):
        # Prefix hash with test namespace
        test_hash = f"{test_namespace}-{config_hash}"
        result = original_add_config(cluster_id, config_json, test_hash, interface_version)
        if result:
    ...
    try:
        ...
        for config_id in client._test_configs:
            try:
                client.supabase.table("sweep_configs").delete().eq("id", config_id).execute()
            except Exception:
                pass
        ...
        for cluster_id in client._test_clusters:
            try:
                client.supabase.table("sweep_config_clusters").delete().eq("id", cluster_id).execute()
            except Exception:
                pass
...
class TestSupabaseIsolated:
    ...
    def test_concurrent_workers_isolated(self, isolated_supabase_client):
        ...
        config = client.add_sweep_config(cluster["id"], {"test": True}, "hash-concurrent")
