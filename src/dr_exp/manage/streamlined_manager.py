...
class StreamlinedManager:
    ...
    def check_stale_jobs(self) -> None:
        ...
        if successful_failures:
            logging.info("Marked %d jobs as failed: %s", len(successful_failures), successful_failures)
        if failed_failures:
            logging.error("Failed to mark %d jobs as failed: %s", len(failed_failures), failed_failures)
    ...
    def check_idle_timeout(self) -> None:
        ...
        if running_jobs:
            ...
            self.last_activity = datetime.now(UTC)
            logging.debug("Found %d running jobs, resetting idle timer", len(running_jobs))
            return
        ...
        if self.job_db.has_queued_jobs():
            ...
            logging.info(
                ...
                len(queue_summary),
                [{"id": job.get("id"), "priority": job.get("priority", 100)} for job in queue_summary]
            )
        else:
        ...
        if datetime.now(UTC) - self.last_activity > self.idle_timeout:
            logging.info("Idle timeout reached (%s minutes), shutting down", self.idle_timeout.total_seconds() / 60)
            self.shutdown = True
