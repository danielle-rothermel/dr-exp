...
class JobExecutor:
    ...
    def execute(self, work_dir: str) -> str:
        ...
        if cfg is None:
            self.client.record_failure(self.job_id, "config_missing", "Config not found")
        ...
        heartbeat_manager = HeartbeatManager(self.client, self.job_id, self.heartbeat_interval)
    ...
    def _finalize_and_upload(
        ...
        bundle_upload = self._create_and_upload_bundle(logger, work_dir, worker_log_path)
    ...
    def _create_and_upload_bundle(self, logger: BaseLogger, work_dir: str, worker_log_path: str) -> dict:
...
def run_worker(
    ...
    # Claim a job
    job = _claim_job(client, worker_id, target_job_id, max_claim_attempts, respect_reservations)
