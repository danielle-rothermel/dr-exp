...
class SupabaseClient:
    ...
    def claim_job(self, worker_id: str = "unassigned_worker") -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # END;
            # $$ LANGUAGE plpgsql;
            response = self.supabase.rpc("claim_next_job", {"worker_id_input": worker_id}).execute()
            if response.data:
    ...
    def update_job(self, job_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.supabase.table("jobs").update(data).eq("id", job_id).execute()
            if response.data:
    ...
    def get_job_details(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.supabase.table("jobs").select("*").eq("id", job_id).maybe_single().execute()
    ...
    def get_config_for_job(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if job_details and job_details.get("config_id"):
                ...
                response = self.supabase.table("sweep_configs").select("config_json").eq("id", config_id).single().execute()
                if response.data:
    ...
    def log_metrics_file(self, job_id: str, local_metrics_file_path: str, remote_metrics_filename: str = "metrics.jsonl") -> Dict[str, Any]:
        """
        ...
        if not os.path.exists(local_metrics_file_path):
            return {"success": False, "message": f"Local metrics file not found: {local_metrics_file_path}"}
        ...
        try:
            with open(local_metrics_file_path, 'rb') as f:
                self.supabase.storage.from_("experiment-artifacts").upload(
                    file=f,
                    path=remote_db_path,
                    file_options={"cache-control": "3600", "upsert": True} # upsert=True to overwrite if exists
                )
    ...
    def record_failure(self, job_id: str, error_type: str, message: str, stacktrace: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.update_job(job_id, {"status": "failed", "end_time": datetime.now(timezone.utc).isoformat()})
            return {"success": True}
        except Exception as e:
    ...
    def upload_artifact(self, job_id: str, local_path: str, remote_path_suffix: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(local_path):
            return {"success": False, "message": f"Local artifact not found: {local_path}"}

        # Determine full remote path based on suffix (root file vs. under 'artifacts/')
        if "/" not in remote_path_suffix and "." in remote_path_suffix: # Simple check for root files like 'worker.log'
        ...
        try:
            if os.path.isdir(local_path):
                ...
                return {"success": False, "message": "Directory upload not directly supported; please zip or upload files individually."}
            else: # It's a file
    ...
    def add_sweep_config_cluster(self, name: str, description: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Adds a new sweep_config_cluster."""
        try:
            data = {"name": name}
            if description is not None:
                data["description"] = description
            response = self.supabase.table("sweep_config_clusters").insert(data).execute()
    ...
    def check_sweep_config_exists(self, config_hash: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.supabase.table("sweep_configs").select("id, config_hash").eq("config_hash", config_hash).maybe_single().execute()
            return response.data if response.data else None
        except Exception as e:
            print(f"Error checking sweep config existence: {e}")
            return None

    def add_sweep_config(self, cluster_id: str, config_json: Dict[str, Any], config_hash: str, interface_version: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    def add_job_entry(self, config_id: str, status: str = "queued", retry_index: int = 0, 
                      interface_version: Optional[str] = None, code_version: Optional[str] = None) -> Optional[Dict[str, Any]]:
