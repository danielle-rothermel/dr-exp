...
class SupabaseMockClient:
            ...
            try:
                with open(job_file_path, 'r+') as f:
                    ...
                    if job_data.get("status") == "queued":
                        ...
                        job_data["assigned_worker"] = f"mock_worker_{uuid.uuid4().hex[:6]}"
                ...
                print(f"Error processing job file {job_file_path}: {e}") # Should go to a logger
                continue
        ...
        try:
            with open(metric_file_path, 'a') as f:
                for metrics in metrics_list:
                    ...
                    if 'timestamp' not in metrics_to_log: # Add timestamp if not present
                        metrics_to_log['timestamp'] = datetime.utcnow().isoformat() + "Z"
    ...
    def upload_artifact(self, job_id: str, local_path: str, remote_path_suffix: str):
        ...
        if "/" not in remote_path_suffix and "." in remote_path_suffix : # Simple check for root files
             destination_path = os.path.join(self.mock_storage_path, f"run_{job_id}", remote_path_suffix)
        else:
             destination_path = os.path.join(self.mock_storage_path, f"run_{job_id}", "artifacts", remote_path_suffix)
        ...
        try:
            if os.path.isdir(local_path):
                # If it's a directory, copy its contents (or the dir itself)
                # For simplicity, let's assume shutil.copytree for dirs, though spec implies individual files
                # The spec for logger implies individual files are logged.
                # For worker upload, it might upload a whole dir.
                # Let's keep it to files for now, or a more complex logic is needed.
                # For now, let's stick to file uploads as per typical artifact logging.
                # If a whole directory needs to be "uploaded", it should be zipped or handled by multiple calls.
                # For simplicity, this mock will copy a single file or an entire directory if local_path is a dir
                if os.path.isdir(local_path):
                     shutil.copytree(local_path, destination_path, dirs_exist_ok=True)
                else: # It's a file
                    shutil.copy2(local_path, destination_path) # copy2 preserves metadata
            # print(f"Uploaded artifact for job {job_id} from {local_path} to {destination_path}") # Debug
    ...
    def add_job(self, job_config: Dict[str, Any], sweep_config_id: str, status: str = "queued") -> Dict[str, Any]:
        """
