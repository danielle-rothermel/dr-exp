    ...
    def run(self, args: Namespace) -> int:
        ...
        try:
            ...
            if os.path.exists(jobs_dir) and os.access(jobs_dir, os.R_OK | os.W_OK):
                checks.append(
                    ("Jobs directory", True, f"Exists and accessible: {jobs_dir}")
                )
            ...
            # Storage directory
            if os.path.exists(config.storage_path) and os.access(
                config.storage_path, os.R_OK | os.W_OK
            ):
                checks.append(
                    (
                        "Storage directory",
                        True,
                        f"Exists and accessible: {config.storage_path}",
                    )
                )
                ...
                checks.append(
                    (
                        "Storage directory",
                        False,
                        f"Not accessible: {config.storage_path}",
                    )
                )
            ...
            try:
                ...
                checks.append(
                    (
                        "Database connectivity",
                        True,
                        f"Successfully connected to {config.mode} database",
                    )
                )
            ...
            try:
                ...
                if stale_jobs:
                    checks.append(
                        ("Stale jobs", False, f"Found {len(stale_jobs)} stale jobs")
                    )
                    print(f"  ⚠️  Warning: {len(stale_jobs)} stale jobs found")
                    if args.verbose:
                        print("  Stale jobs:")
                        for stale_job in stale_jobs[:3]:
                            print(
                                f"    - {stale_job.job_id[:8]}... Worker: {stale_job.assigned_worker} Age: {stale_job.age_seconds}s"
                            )
                else:
            ...
            if config.mode == "files_local":
                ...
                if alternatives:
                    checks.append(
                        (
                            "Alternative job locations",
                            False,
                            f"Found jobs in {len(alternatives)} other locations",
                        )
                    )
                    print("  ⚠️  Found jobs in alternative locations:")
                    ...
                    checks.append(
                        (
                            "Alternative job locations",
                            True,
                            "No jobs found in alternative locations",
                        )
                    )
