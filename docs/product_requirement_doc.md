...
### Phase 1: Mocks, Scaffolding & Core Local Components (Highest Priority)
    - **Supabase Schema Definition:** Implement schema in a development Supabase instance.
    - **`Supabase Mock Client` & `Mock Reset Utility`:** For local, offline testing. Simulates DB and storage operations using local files.
    - **`Mock Trainer`:** Simulates `train()` behavior, including configurable success/failure modes and metric generation.
    - **`StructuredLogger` (Initial):** Writes to unique local paths.
    - **`Config Generator` (Initial):** Generates configs and can "upload" to the `Supabase Mock Client`.
    - **Basic Tests:** Unit tests for logger, mock trainer, and config generator.
### Phase 2: Core System Implementation & Integration
    - **`StructuredLogger` (Full):** Robust local file logging.
    - **`Worker Process`:**
        - Integrates with `Supabase Client` (mock first, then real).
        - Job claiming logic.
        - Runs `Mock Trainer` using `StructuredLogger`.
        - Handles local file uploads to Supabase Storage (mock first, then real).
        - Updates job status in Supabase.
    - **`SLURM Manager`:**
        - Spawns and monitors `Worker Processes` (using `Mock Trainer`).
        - Basic heartbeat monitoring (simulated via mock Supabase).
        - Path generation logic for workers.
    - **`FastAPI Backend` (Initial):**
        - Basic endpoints for job status/logs, interacting with `Supabase Client` (mock first, then real).
        - Admin endpoints for kill/requeue (initially no auth, then basic API key).
    - **Integration Tests:** Test flow from config generation through worker execution using mocks.
### Phase 3: UI & Full System Polish
    - **`React Babysitter UI` (Initial):**
        - Job table view, displays status, basic log/metric view.
