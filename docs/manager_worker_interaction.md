...
This document clarifies how `scripts/run_manager.py` and `run_worker.py` cooperate during a training run. It focuses on the runtime sequence and how data flows between the two scripts.
...
5. **Worker entrypoint**: `run_worker_main()` (defined in `dr_exp.manager`) simply loads `run_worker.run_worker()` from `scripts/run_worker.py` and passes the base path and working directory. If the real worker implementation is missing an error is raised.
