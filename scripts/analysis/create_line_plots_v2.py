#!/usr/bin/env python3
"""Create line plots for experiments with proper hyperparameter handling."""
...
def get_final_val_acc(config_results: Dict[str, List]) -> Dict[str, float]:
    """Get final validation accuracy for each config."""
    final_accs = {}
    for config, runs in config_results.items():
        val_accs = []
        for metrics in runs:
            if metrics:
                val_accs.append(metrics[-1].get("val_acc", 0))
        final_accs[config] = np.mean(val_accs) if val_accs else 0
    return final_accs
...
def create_line_plots_v2(config_results: Dict[str, List], output_dir: Path):
    ...
    # Get final accuracies for sorting
    final_accs = get_final_val_acc(config_results)

    # Sort configs by final accuracy
    sorted_configs = sorted(
        config_results.keys(), key=lambda x: final_accs[x], reverse=True
    )
    ...
    for base, configs in sorted(config_groups.items()):
        ...
        best_config = max(configs, key=lambda x: final_accs[x])
        plotted_configs.append(best_config)

    # Sort by accuracy and plot top 20
    plotted_configs.sort(key=lambda x: final_accs[x], reverse=True)
    ...
    for zoom_type in ["full", "zoomed"]:
        ...
        for i, config in enumerate(plotted_configs):
            ...
            label = f"{shorten_label(config)} ({final_accs[config]:.3f})"
        ...
        plt.title("Top Performers - Best Hyperparameters per Config Type", fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / f"best_per_config_type_val_acc_{zoom_type}.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
    ...
    for zoom_type in ["full", "zoomed"]:
        ...
        plt.ylabel("Validation Accuracy", fontsize=12)
        plt.title(
            f"All Configurations with Hyperparameter Variations ({num_configs} configs)",
            fontsize=14,
        )
        plt.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / f"all_configs_val_acc_v2_{zoom_type}.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
    ...
    ax.set_title("SGD Learning Rate Comparison")
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Validation Accuracy")
    ax.set_title("AdamW Hyperparameter Comparison")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Validation Accuracy")
    ax.set_title("Controlled Experiments (Fixed Augmentation)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    ...
    for default, lower in pairs:
        if default in final_accs and lower in final_accs:
            lr_default.append(final_accs[default])
            lr_lower.append(final_accs[lower])
        else:
            lr_default.append(0)
            lr_lower.append(0)
    ...
    ax.set_title("Learning Rate Impact on Final Accuracy")
    ax.set_xticks(x_pos)
    ax.set_xticklabels(
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "hyperparameter_comparisons.png", dpi=150, bbox_inches="tight"
    )
    plt.close()

    print(f"Created plots in {output_dir}")
    print("- best_per_config_type_val_acc_full.png: Best hyperparameters (full y-axis)")
    print(
        "- best_per_config_type_val_acc_zoomed.png: Best hyperparameters (zoomed y-axis)"
    )
    print(
        f"- all_configs_val_acc_v2_full.png: All {num_configs} configurations (full y-axis)"
    )
    print(
        f"- all_configs_val_acc_v2_zoomed.png: All {num_configs} configurations (zoomed y-axis)"
    )
    print("- hyperparameter_comparisons.png: Detailed hyperparameter impact analysis")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create line plots with proper hyperparameter handling"
    )
    parser.add_argument("--base-path", type=Path, default=Path("."))
    parser.add_argument("--experiment", type=str, default="cluster_t0")
    parser.add_argument(
        "--output-dir", type=Path, default=Path("./na_full_t1/01_line_plots_v2")
    )
