#!/usr/bin/env python3
"""Analyze early training dynamics (first 10 epochs) and correlation with best validation accuracy.

This version uses the best (maximum) validation accuracy across all epochs as the performance metric.
"""
...
def analyze_config_early_regression(config_runs: List[Dict]) -> Dict:
    ...
    return {
        ...
        "best_val_acc": np.mean(all_best_val_accs),
...
def create_early_correlation_plots(
    ...
    """Create correlation plots between early regression metrics and best validation accuracy."""
    fig, axes = plt.subplots(3, 3, figsize=(15, 15))
    fig.suptitle(
        "Early Training Regression Metrics (First 10 Epochs) vs Best Validation Accuracy",
        fontsize=16,
    )
    ...
    # Plot training metrics vs best val acc
    for i, (metric, label) in enumerate(metrics):
        ax = axes[0, i]

        # Calculate correlation
        corr, p_value = stats.pearsonr(df[metric], df["best_val_acc"])

        # Create scatter plot
        scatter = ax.scatter(
            df[metric], df["best_val_acc"], c=df.index, cmap="viridis", s=100, alpha=0.7
        )

        # Add regression line
        z = np.polyfit(df[metric], df["best_val_acc"], 1)
        p = np.poly1d(z)
    ...
    # Plot validation metrics vs best val acc
    val_metrics = [
    ...
    for i, (train_metric, val_metric, label) in enumerate(train_val_pairs):
        ...
        cbar.set_label("Best Val Acc", rotation=270, labelpad=15)
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze early regression correlations using best validation accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="early_correlation_analysis_best_acc",
    ...
    # Create DataFrame and sort by best validation accuracy
    df = pd.DataFrame(results)
    df = df.sort_values("best_val_acc", ascending=False)

    # Save summary
    summary_path = output_dir / "early_regression_analysis_summary_best_acc.csv"
    df.to_csv(summary_path, index=False)
    print(f"\nSaved summary to {summary_path}")

    # Create plots with labels
    plot_path_labeled = (
        output_dir / "early_regression_correlation_analysis_best_acc_labeled.png"
    )
    ...
    plot_path_unlabeled = (
        output_dir / "early_regression_correlation_analysis_best_acc_unlabeled.png"
    )
    ...
    print("\nTop 10 configurations by best validation accuracy:")
    print(
        df[["config", "best_val_acc", "val_slope", "val_r2"]]
        .head(10)
        .to_string(index=False)
    )

    print("\nCorrelation summary (early metrics vs best accuracy):")
    for metric in [
        "train_slope",
        "train_r2",
        "train_rmse",
