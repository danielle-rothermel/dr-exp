#!/usr/bin/env python3
"""Sweep over different numbers of early epochs for regression analysis using best validation loss."""
...
def sweep_early_epochs(
    ...
    for max_epoch in epoch_range:
        ...
        if results:
            ...
            df = df.sort_values("best_val_loss", ascending=True)  # Lower is better
...
def create_correlation_sweep_plots(
    ...
    for epoch in epochs_list:
        ...
        for metric in metrics:
            if (
                ...
                corr, p_val = stats.pearsonr(df[metric], df["best_val_loss"])
    ...
    fig.suptitle(
        "Correlation with Best Val Loss vs Number of Early Epochs Used", fontsize=16
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / "correlation_vs_early_epochs_best_loss.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    ax.set_ylabel("Correlation with Best Val Loss", fontsize=12)
    ax.set_title(
        "Strongest Predictor at Each Early Epoch Cutoff (Best Val Loss)", fontsize=14
    )
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / "best_predictor_vs_early_epochs_best_loss.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    summary_df.to_csv(
        output_path / "early_epochs_sweep_summary_best_loss.csv", index=False
    )
    print(
        f"\nSaved summary to {output_path / 'early_epochs_sweep_summary_best_loss.csv'}"
    )
...
def create_heatmap_plot(results_by_epoch: Dict[int, pd.DataFrame], output_path: Path):
    ...
    # Focus on top performers (by best val loss - lower is better)
    best_losses = {}
    for config in all_configs:
        # Get best loss from any epoch's data
        for df in results_by_epoch.values():
            if config in df["config"].values:
                best_losses[config] = df[df["config"] == config][
                    "best_val_loss"
                ].values[0]
                break

    # Get top 15 configs (sorted by loss - ascending)
    top_configs = sorted(best_losses.keys(), key=lambda x: best_losses[x])[:15]
    ...
    plt.suptitle(
        "Top 15 Configurations (by Best Val Loss): Metric Values vs Early Epoch Cutoff",
        fontsize=16,
    )
    plt.tight_layout()
    plt.savefig(
        output_path / "metrics_heatmap_vs_early_epochs_best_loss.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep early epochs for regression analysis using best validation loss"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="na_full_t1_best_loss/early_epochs_sweep",
    ...
    print("Files created:")
    print(
        "- correlation_vs_early_epochs_best_loss.png: How correlations change with epoch cutoff"
    )
    print(
        "- best_predictor_vs_early_epochs_best_loss.png: Which metric is best at each cutoff"
    )
    print(
        "- metrics_heatmap_vs_early_epochs_best_loss.png: Heatmap of top configs across cutoffs"
    )
    print("- early_epochs_sweep_summary_best_loss.csv: Detailed correlation data")
