#!/usr/bin/env python3
"""Create a comprehensive summary of experiment insights using best validation accuracy."""
...
def load_analysis_results(output_dir: Path) -> Dict:
    ...
    # Load early correlation analysis best acc
    early_csv = output_dir / "early_regression_analysis_summary_best_acc.csv"
    if early_csv.exists():
        results["early"] = pd.read_csv(early_csv)

    # Load full correlation analysis best acc
    full_csv = output_dir / "regression_analysis_summary_best_acc.csv"
    if full_csv.exists():
...
def create_insights_summary(results: Dict, output_dir: Path):
    ...
    insights_dir = output_dir / "00_experiment_insights_best_acc"
    ...
    # Top 15 by best accuracy
    top_configs = early_df.nlargest(15, "best_val_acc")
    ax1.barh(
        range(len(top_configs)),
        top_configs["best_val_acc"],
    ...
    ax1.set_title("Top 15 Configurations by Best Val Accuracy")
    ax1.grid(True, alpha=0.3)

    # Add accuracy values on bars
    for i, (idx, row) in enumerate(top_configs.iterrows()):
        ax1.text(
            row["best_val_acc"] - 0.002,
            i,
    ...
    category_stats = (
        ...
        .agg({"best_val_acc": ["mean", "std", "max", "count"]})
        .round(4)
    )
    ...
    ax2.set_title("Performance by Configuration Category (Best Val Acc)")
    ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(
        insights_dir / "top_performers_summary_best_acc.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    # Early vs best accuracy scatter
    scatter = ax1.scatter(
        early_df["val_slope"],
        early_df["best_val_acc"],
    ...
    ax1.set_title("Early Slope vs Best Performance")
    ax1.grid(True, alpha=0.3)
    ...
    if not controlled.empty:
        ...
        ax3.set_title("Controlled Experiments - Best Val Acc")
    ...
    plt.tight_layout()
    plt.savefig(
        insights_dir / "early_dynamics_analysis_best_acc.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    with open(insights_dir / "experiment_insights_best_acc.txt", "w") as f:
        f.write("EXPERIMENT INSIGHTS SUMMARY - BEST VALIDATION ACCURACY\n")
        f.write("=====================================================\n\n")

        f.write("1. TOP PERFORMING CONFIGURATIONS (BY BEST VAL ACC)\n")
        f.write("-" * 50 + "\n")
        for idx, row in top_configs.head(10).iterrows():
            f.write(
                f"{row['config']}: {row['best_val_acc']:.4f} (val_slope: {row['val_slope']:.3f})\n"
            )

        f.write("\n2. CATEGORY PERFORMANCE SUMMARY (BEST VAL ACC)\n")
        f.write("-" * 50 + "\n")
        f.write(category_stats.to_string())

        f.write("\n\n3. EARLY PREDICTION CORRELATIONS WITH BEST ACC\n")
        f.write("-" * 50 + "\n")
        if "early" in results:
            from scipy import stats

            for metric in ["train_slope", "train_r2", "val_slope", "val_r2"]:
                corr, p_val = stats.pearsonr(early_df[metric], early_df["best_val_acc"])
                f.write(f"{metric}: r = {corr:.3f}, p = {p_val:.3f}\n")

        f.write("\n4. KEY FINDINGS (BEST VAL ACC)\n")
        f.write("-" * 50 + "\n")
        ...
        if not hp_configs.empty:
            ...
            if not best_adamw.empty:
                f.write(
                    f"- Best AdamW setting: {best_adamw.iloc[0]['config']} (best acc: {best_adamw.iloc[0]['best_val_acc']:.4f})\n"
                )
            if not best_sgd.empty:
                f.write(
                    f"- Best SGD setting: {best_sgd.iloc[0]['config']} (best acc: {best_sgd.iloc[0]['best_val_acc']:.4f})\n"
                )

        # Controlled experiment insights
        if not controlled.empty:
            f.write(
                "\n- Controlled Experiments (fixed augmentation, by best val acc):\n"
            )
            controlled_sorted = controlled.sort_values("best_val_acc", ascending=False)
            for idx, row in controlled_sorted.iterrows():
                f.write(f"  {row['config']}: {row['best_val_acc']:.4f}\n")

        f.write("\n5. RECOMMENDATIONS (BASED ON BEST VAL ACC)\n")
        f.write("-" * 50 + "\n")
        f.write(
            "- Early validation loss slope still correlates with best achieved accuracy\n"
        )
        f.write("- Best validation accuracy may occur before final epoch\n")
        f.write("- Hyperparameter tuning can improve peak performance significantly\n")
        f.write(
            "- Some configurations achieve high peak accuracy but may overfit later\n"
        )

    print(f"\nInsights summary saved to {insights_dir}/")
    print("- experiment_insights_best_acc.txt: Detailed text summary")
    print(
        "- top_performers_summary_best_acc.png: Visual summary of best configurations"
    )
    print(
        "- early_dynamics_analysis_best_acc.png: Analysis of early training predictive power"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Create experiment insights summary using best validation accuracy"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="correlation_analysis_best_acc",
    ...
    # Load results
    print("Loading analysis results for best validation accuracy...")
