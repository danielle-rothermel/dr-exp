#!/usr/bin/env python3
"""Create line plots for experiments using best validation accuracy."""
...
def get_best_val_acc(config_results: Dict[str, List]) -> Dict[str, float]:
    """Get best validation accuracy for each config."""
    best_accs = {}
    for config, runs in config_results.items():
        val_accs = []
        for metrics in runs:
            if metrics:
                # Find maximum validation accuracy across all epochs
                max_acc = max(m.get("val_acc", 0) for m in metrics)
                val_accs.append(max_acc)
        best_accs[config] = np.mean(val_accs) if val_accs else 0
    return best_accs
...
def create_line_plots_v2(config_results: Dict[str, List], output_dir: Path):
    ...
    # Get best accuracies for sorting
    best_accs = get_best_val_acc(config_results)

    # Sort configs by best accuracy
    sorted_configs = sorted(
        config_results.keys(), key=lambda x: best_accs[x], reverse=True
    )
    ...
    for base, configs in sorted(config_groups.items()):
        # Get best config from this group
        best_config = max(configs, key=lambda x: best_accs[x])
        plotted_configs.append(best_config)

    # Sort by accuracy and plot top 20
    plotted_configs.sort(key=lambda x: best_accs[x], reverse=True)
    ...
    for zoom_type in ["full", "zoomed"]:
        ...
        for i, config in enumerate(plotted_configs):
            epochs, _, _, _, mean_val_acc, _, _, _, std_val_acc = aggregate_metrics(
                config_results[config]
            )
            label = f"{shorten_label(config)} (best: {best_accs[config]:.3f})"

            plt.plot(epochs, mean_val_acc, label=label, linewidth=2, alpha=0.8)
            plt.fill_between(
                epochs,
            ...
            min_acc = min(min_acc, np.min(mean_val_acc - std_val_acc))
            max_acc = max(max_acc, np.max(mean_val_acc + std_val_acc))

        plt.xlabel("Epoch", fontsize=12)
        plt.ylabel("Validation Accuracy", fontsize=12)
        plt.title("Top Performers - Best Val Acc per Config Type", fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / f"best_per_config_type_best_val_acc_{zoom_type}.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
    ...
    for zoom_type in ["full", "zoomed"]:
        ...
        plt.ylabel("Validation Accuracy", fontsize=12)
        plt.title(
            f"All Configurations - Sorted by Best Val Acc ({num_configs} configs)",
            fontsize=14,
        )
        plt.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / f"all_configs_best_val_acc_{zoom_type}.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
    ...
    for config in sgd_configs[:10]:  # Limit to 10 for clarity
        ...
        label = f"{shorten_label(config)} (best: {best_accs[config]:.3f})"
        line = ax.plot(epochs, mean_val_acc, label=label, linewidth=2, alpha=0.8)[0]
        ax.fill_between(
            epochs,
            mean_val_acc - std_val_acc,
            mean_val_acc + std_val_acc,
    ...
    ax.set_title("SGD Learning Rate Comparison (Best Val Acc)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)

    # AdamW hyperparameter comparison
    ax = axes[0, 1]
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Validation Accuracy")
    ax.set_title("AdamW Hyperparameter Comparison (Best Val Acc)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Validation Accuracy")
    ax.set_title("Controlled Experiments - Best Val Acc")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)

    # Learning rate impact across steps
    ax = axes[1, 1]
    ...
    for default, lower in pairs:
        if default in best_accs and lower in best_accs:
            lr_default.append(best_accs[default])
            lr_lower.append(best_accs[lower])
        else:
            lr_default.append(0)
            lr_lower.append(0)
    ...
    ax.set_title("Learning Rate Impact on Best Accuracy")
    ax.set_xticks(x_pos)
    ax.set_xticklabels(
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "hyperparameter_comparisons_best_acc.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()
    ...
    print(
        "- best_per_config_type_best_val_acc_full.png: Best hyperparameters (full y-axis)"
    )
    print(
        "- best_per_config_type_best_val_acc_zoomed.png: Best hyperparameters (zoomed y-axis)"
    )
    print(
        f"- all_configs_best_val_acc_full.png: All {num_configs} configurations (full y-axis)"
    )
    print(
        f"- all_configs_best_val_acc_zoomed.png: All {num_configs} configurations (zoomed y-axis)"
    )
    print(
        "- hyperparameter_comparisons_best_acc.png: Detailed hyperparameter impact analysis"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create line plots using best validation accuracy"
    )
    parser.add_argument("--base-path", type=Path, default=Path("."))
    parser.add_argument("--experiment", type=str, default="cluster_t0")
    parser.add_argument(
        "--output-dir", type=Path, default=Path("./na_full_t1_best_acc/01_line_plots")
    )
