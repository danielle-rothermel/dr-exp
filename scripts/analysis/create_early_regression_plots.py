#!/usr/bin/env python3
"""Create training loss regression plots using only first 10 epochs for fitting."""
...
def aggregate_metrics(
    ...
    """Aggregate train and val loss across seeds."""
    ...
    return epochs, mean_train_loss, mean_val_loss
...
def create_grouped_plots(config_results: Dict[str, List], output_dir: Path):
    """Create grouped training and validation loss plots with regression lines."""
    ...
    for group_idx, (group_name, group_configs) in enumerate(groups):
        ...
        for i, config in enumerate(group_configs):
            epochs, mean_train_loss, _ = aggregate_metrics(config_results[config])
            ...
            # Fit regression on first 10 epochs
            ...
            if not np.isnan(slope):
                ...
                epochs_line = np.logspace(0, np.log10(10), 100)
        ...
        # Add vertical line at epoch 10
        ax.axvline(x=10, color="gray", linestyle=":", alpha=0.5, linewidth=2)
        ax.text(
            10,
            ax.get_ylim()[1] * 0.95,
            "Regression fit →",
    ...
    plt.suptitle(
        "Training Loss with Log-Linear Regression (First 10 Epochs)", fontsize=14
    )
    plt.tight_layout()
    plt.savefig(
        output_dir / "grouped_train_loss_regression_10epochs.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()
    ...
    for group_idx, (group_name, group_configs) in enumerate(groups):
        ...
        for i, config in enumerate(group_configs):
            epochs, _, mean_val_loss = aggregate_metrics(config_results[config])
            label = config_labels.get(config, config)
            color_idx = sorted_configs.index(config)
    ...
    plt.suptitle(
        "Validation Loss with Log-Linear Regression (First 10 Epochs)", fontsize=14
    )
    plt.tight_layout()
    plt.savefig(
        output_dir / "grouped_val_loss_regression_10epochs.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()

    # Create individual detailed plots with statistics
    for loss_type in ["train", "val"]:
        for group_idx, (group_name, group_configs) in enumerate(groups):
            ...
            for i, config in enumerate(group_configs):
                epochs, mean_train_loss, mean_val_loss = aggregate_metrics(
                    config_results[config]
                )
                mean_loss = mean_train_loss if loss_type == "train" else mean_val_loss
                label = config_labels.get(config, config)
                color_idx = sorted_configs.index(config)
            ...
            plt.ylabel(
                f"{'Training' if loss_type == 'train' else 'Validation'} Loss",
                fontsize=14,
            )
            plt.title(
                f"{group_name}\n{'Training' if loss_type == 'train' else 'Validation'} Loss with Log-Linear Regression",
                fontsize=16,
            )
            ...
            stats_text = "Regression Statistics (epochs 1-10):\n"
            stats_text += "-" * 35 + "\n"
            ...
            # Add vertical line at epoch 10
            plt.axvline(x=10, color="gray", linestyle=":", alpha=0.5, linewidth=2)
            plt.text(
                10,
            ...
            plt.savefig(
                output_dir
                / f"group{group_idx + 1}_{loss_type}_loss_regression_10epochs_detailed.png",
                dpi=150,
                bbox_inches="tight",
            )
            plt.close()

    print(f"Created early regression plots (10 epochs) in {output_dir}")
    print("Files created:")
    print("- grouped_train_loss_regression_10epochs.png")
    print("- grouped_val_loss_regression_10epochs.png")
    print("- group[1-3]_train_loss_regression_10epochs_detailed.png")
    print("- group[1-3]_val_loss_regression_10epochs_detailed.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create early training regression plots (10 epochs)"
    )
    parser.add_argument("--base-path", type=Path, default=Path("."))
    parser.add_argument("--experiment", type=str, default="cluster_t0")
    parser.add_argument(
        "--output-dir", type=Path, default=Path("./early_regression_plots")
    )
    ...
    print("Fitting regressions to first 10 epochs only...")
