#!/usr/bin/env python3
"""Create comprehensive experiment summary CSV and subplot grids with filtered/renumbered steps."""
...
def get_config_changes(config_name: str) -> Dict[str, Any]:
    ...
    changes = {
        ...
        # Skip steps 8-10 (LRN issues)
        "step11_resnet12": {"model": "resnet12"},
        "step12_alexnet": {"model": "alexnet"},
...
def renumber_step_name(config_name: str) -> str:
    ...
    # Mapping from old step numbers to new ones
    renumber_map = {
        "step00": "step00",
        "step01": "step01",
        "step02": "step02",
    ...
    # Extract step number and suffix
...
def collect_experiment_data(base_path: Path, experiment: str) -> pd.DataFrame:
    ...
    for config_name, results in config_results.items():
        ...
        # Get config changes using original name
        original_name = results[0]["original_name"]
        changes = get_config_changes(original_name)
...
def create_subplot_grids(df: pd.DataFrame, output_dir: Path):
    ...
    # We have 15 configs now (removed 3), so use 5x3 grid
    n_rows, n_cols = 3, 5

    # Pad with empty if needed
    while len(configs) < n_rows * n_cols:
        configs.append(None)

    # Create figure 1: Loss plots (linear scale)
    fig1, axes1 = plt.subplots(
        n_rows, n_cols, figsize=(25, 12), sharex=True, sharey=True
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("./experiment_summary_filtered")
    )

    args = parser.parse_args()

    print(f"Analyzing experiments in {args.base_path / args.experiment}")
    print("Filtering out steps 8-10 (LRN-related issues) and renumbering...")
