#!/usr/bin/env python3
"""Analyze early training dynamics (first 10 epochs) and correlation with final performance.
...
def create_early_correlation_plots(
    ...
    """Create correlation plots between early regression metrics and final validation accuracy."""
    fig, axes = plt.subplots(3, 3, figsize=(15, 15))
    fig.suptitle(
        "Early Training Regression Metrics (First 10 Epochs) vs Final Validation Accuracy",
        fontsize=16,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze early regression correlations with hyperparameter handling"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="early_correlation_analysis",
    ...
    # Save summary
    summary_path = output_dir / "early_regression_analysis_summary_v2.csv"
    df.to_csv(summary_path, index=False)
    print(f"\nSaved summary to {summary_path}")

    # Create plots with labels
    plot_path_labeled = (
        output_dir / "early_regression_correlation_analysis_v2_labeled.png"
    )
    ...
    plot_path_unlabeled = (
        output_dir / "early_regression_correlation_analysis_v2_unlabeled.png"
    )
    ...
    print("\nCorrelation summary (early metrics vs final accuracy):")
    for metric in [
        "train_slope",
        "train_r2",
        "train_rmse",
