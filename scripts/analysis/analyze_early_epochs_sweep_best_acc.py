#!/usr/bin/env python3
"""Sweep over different numbers of early epochs for regression analysis using best validation accuracy."""
...
def create_correlation_sweep_plots(
    ...
    for epoch in epochs_list:
        ...
        for metric in metrics:
            if (
                ...
                corr, p_val = stats.pearsonr(df[metric], df["best_val_acc"])
    ...
    fig.suptitle(
        "Correlation with Best Val Accuracy vs Number of Early Epochs Used", fontsize=16
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / "correlation_vs_early_epochs_best_acc.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    ax.set_ylabel("Correlation with Best Val Accuracy", fontsize=12)
    ax.set_title(
        "Strongest Predictor at Each Early Epoch Cutoff (Best Val Acc)", fontsize=14
    )
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / "best_predictor_vs_early_epochs_best_acc.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    summary_df.to_csv(
        output_path / "early_epochs_sweep_summary_best_acc.csv", index=False
    )
    print(
        f"\nSaved summary to {output_path / 'early_epochs_sweep_summary_best_acc.csv'}"
    )
...
def create_heatmap_plot(results_by_epoch: Dict[int, pd.DataFrame], output_path: Path):
    ...
    for config in all_configs:
        # Get best accuracy from any epoch's data
        for df in results_by_epoch.values():
            if config in df["config"].values:
                best_accs[config] = df[df["config"] == config]["best_val_acc"].values[0]
                break

    # Get top 15 configs
    top_configs = sorted(best_accs.keys(), key=lambda x: best_accs[x], reverse=True)[
        :15
    ]
    ...
    plt.suptitle(
        "Top 15 Configurations (by Best Val Acc): Metric Values vs Early Epoch Cutoff",
        fontsize=16,
    )
    plt.tight_layout()
    plt.savefig(
        output_path / "metrics_heatmap_vs_early_epochs_best_acc.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep early epochs for regression analysis using best validation accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="na_full_t1_best_acc/early_epochs_sweep",
    ...
    print("Files created:")
    print(
        "- correlation_vs_early_epochs_best_acc.png: How correlations change with epoch cutoff"
    )
    print(
        "- best_predictor_vs_early_epochs_best_acc.png: Which metric is best at each cutoff"
    )
    print(
        "- metrics_heatmap_vs_early_epochs_best_acc.png: Heatmap of top configs across cutoffs"
    )
    print("- early_epochs_sweep_summary_best_acc.csv: Detailed correlation data")
