-- Fix remote database schema
-- This script ensures all tables exist properly

-- First, drop existing partial tables
DROP TABLE IF EXISTS sync_status CASCADE;
DROP TABLE IF EXISTS jobs CASCADE;
DROP TABLE IF EXISTS experiments CASCADE;

-- Drop views if they exist
DROP VIEW IF EXISTS job_queue;
...
-- Grant permissions
GRANT ALL ON experiments TO service_role;
GRANT ALL ON jobs TO service_role;
GRANT ALL ON sync_status TO service_role;
GRANT ALL ON job_queue TO service_role;
GRANT ALL ON experiment_stats TO service_role;

-- Test that everything works
DO $$
BEGIN
    -- Verify tables exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'experiments') THEN
        RAISE EXCEPTION 'experiments table not created';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jobs') THEN
        RAISE EXCEPTION 'jobs table not created';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'sync_status') THEN
        RAISE EXCEPTION 'sync_status table not created';
    END IF;
