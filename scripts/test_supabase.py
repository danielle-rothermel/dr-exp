...
def check_supabase_running():
    ...
    try:
        ...
        return result.returncode == 0 and "API URL: http://127.0.0.1:54321" in result.stdout
...
def main():
    ...
    parser.add_argument(
        "--reset-db",
        action="store_true",
        help="Reset database before running tests"
    )
    parser.add_argument(
        "--no-start",
        action="store_true",
        help="Don't automatically start Supabase"
    )
    parser.add_argument(
        "test_path",
        nargs="?",
        help="Custom test path to run"
    )
    ...
    if not args.no_start:
        ...
        print("❌ Supabase is not running. Start it with 'supabase start' or remove --no-start")
        sys.exit(1)
