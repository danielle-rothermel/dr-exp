...
def main():
    parser = argparse.ArgumentParser(description="Submit chronological ablation experiments")
    ...
    print(f"  Existing jobs to skip: {len(experiments) * len(args.seeds) - total_new_jobs}")
    ...
    for i, (config_name, seed, priority) in enumerate(jobs_to_submit, 1):
        print(f"[{i}/{total_new_jobs}] Submitting {config_name}, seed={seed}, priority={priority}...", end=" ")
