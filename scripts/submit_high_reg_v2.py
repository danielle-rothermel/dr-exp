...
def main():
    parser = argparse.ArgumentParser(description="Submit high regularization experiments")
    ...
    parser.add_argument("--force", action="store_true",
                      help="Submit all jobs even if they exist")
    ...
    print(f"  Configurations: {', '.join(c.replace('.yaml', '') for c, _ in high_reg_configs)}")
    print(f"  Seeds per config: {len(args.seeds)} ({args.seeds})")
    print(f"  Total possible jobs: {len(high_reg_configs) * len(args.seeds)}")
    print(f"  Existing jobs to skip: {len(high_reg_configs) * len(args.seeds) - total_new_jobs}")
    ...
    for i, (config_name, seed, priority) in enumerate(jobs_to_submit, 1):
        print(f"[{i}/{total_new_jobs}] {config_name} seed={seed} priority={priority}", end=" ")
    ...
    if not args.dry_run and successful > 0:
        ...
        print("\n📊 Current job status:")
        subprocess.run([
            "dr_exp", "--base-path", str(args.base_path),
            "--experiment", args.experiment, "status"
        ])
